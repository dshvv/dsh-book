 ...
 */
export const generateActionCreator = (type) => {
...
import { generateActionCreator } from "./redux-helper";

// generateActionCreator 来创建type为SET_MSG 的actionCreator
export const setMsg = generateActionCreator("SET_MSG");
```
...
```javascript title="src/store/redux-helper.js"
/**
 * reducer 的辅助工具,生成一个reducer
 * @param handlers修改状态的逻辑函数，initialState为状态初始值
 * @return 返回一个reducer函数
 */
export const generateReducer = (handlers, initialState) => {
...
import { createReducerHelper } from "./redux-helper";
...
// reducer
const reducer = createReducerHelper({
...
**总结**     
如上说的优化，其实就是提供了两个生成器，一个是`generateActionCreator`,一个是`generateReducer`。
