...
Redux 是一个独立的 JS 库。即使您没有设置用户界面，您也可以创建和使用 Redux Store。这也意味着您可以将 Redux 与任何 UI 框架（甚至没有任何UI 框架）一起使用，并在客户端和服务器上使用它。您可以使用 React、Vue、Angular、Ember、jQuery 编写 Redux 应用程序。
